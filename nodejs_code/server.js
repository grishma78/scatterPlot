...
app.get('/getheightsal', function (request, response) {
   response.send(JSON.stringify(sp.arrange(parseInt(request.param('depid')), jsonEmp, jsonDept, jsonPos, jsonPosDept, jsonPosDeptSal, jsonEmpDeptPos, app, response)));
});


app.get('/getDepartment', function (request, response) {
   response.send(JSON.stringify(jsonDept));
 });
